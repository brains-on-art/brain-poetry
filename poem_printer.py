...
def parse_poem(raw_poem, linewidth=50, indent='       '):
    # Removes white space and empty lines, splits and indents lines that are too long
    ...
    for line in raw_poem:
        if len(line) <= linewidth:
            ...
            [res.append(x) for x in textwrap.wrap(line, width=linewidth, subsequent_indent=indent)]
    return '\n'.join(res)

def print_poem(raw_poem, username):
    ser = serial.Serial('/dev/ttyACM0', baudrate=9600, timeout=3.0) # jari koski tähän! 13/5/2014
    time.sleep(2.0)
...
#    	line=line.replace("ß","\xE1")
    ...
    print 'Printing username', username, type(username  )
